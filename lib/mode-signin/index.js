...
SigninPanel.prototype.bindAll = function() {
  ...
  if(options.recaptcha && options.recaptcha.required) {
    this.query('button').attr('disabled', true);
    this.recaptcha = new Recaptcha(this.query('.a0-recaptcha')[0], options.recaptcha);
    this.recaptcha.onVerify = function(token) {
      self.query('button').attr('disabled', false);
    }
  }
