...
function SigninPanel(widget, options) {
  ...
  this.recaptcha = options.recaptcha.key?new Recaptcha(options.recaptcha):null;
...
/**
 * Enables/disables the recaptcha widget
 *
...
SigninPanel.prototype.requireRecaptcha = function(required) {
  var self = this;

  var options = this.options;

  var recaptcha_container = this.query('.a0-recaptcha');
  var submit_button =  this.query('button[type=submit]');
  var recaptcha_holder = this.query('input[name=recaptcha-token]');

  if (required) {
    submit_button.attr('disabled', true);
    this.recaptcha.show(recaptcha_container[0], function(token) {
      recaptcha_holder.val(token);
